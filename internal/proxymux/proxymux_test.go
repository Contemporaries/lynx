...
func TestProxyMuxUDPAssociate(t *testing.T) {
	...
	defer openCancel()
	assoc, err := pool.AssociateUDP(openCtx)
	if err != nil {
		t.Fatal(err)
	}
	defer assoc.Close()
	...
	if _, err := assoc.WriteTo(payload, echo.LocalAddr().String()); err != nil {
		t.Fatal(err)
	}
	got := make([]byte, 64)
	done := make(chan error, 1)
	go func() {
		n, addr, err := assoc.ReadFrom(got)
		if err != nil {
			done <- err
			return
		}
		if string(got[:n]) != string(payload) {
			done <- fmt.Errorf("got %q from %s", got[:n], addr)
			return
		}
		done <- nil
	}()
	...
	case <-time.After(3 * time.Second):
		t.Fatal("timeout waiting for udp reply")
	}
}
